...
def predict():
    if request.method == 'POST':
        ...
        Departure_min = pd.to_datetime(
            dep_time, format="%Y-%m-%dT%H:%M").minute

        arrival_time = request.form['Arrival_Time']
        Arrival_hour = pd.to_datetime(
            arrival_time, format="%Y-%m-%dT%H:%M").hour
        Arrival_min = pd.to_datetime(
            arrival_time, format="%Y-%m-%dT%H:%M").minute
        ...
        return render_template('home.html', predictions='You would have to Pay approximately Rs. {} for one ticket.'.format(output))
